/**
 * PDF-import — stap 1 (debug): toont enkel het resultaat van klas- en
 * tekstherkenning uit een geüploade Smartschool-klaslijst-PDF.
 *
 * Schrijft NIETS naar Dexie en raakt de bestaande CSV-import (classes.csv /
 * students.csv / foto's) niet aan — puur ter verificatie, vóór de volgende
 * stappen (foto-extractie, naam-matching, Dexie-integratie) gebouwd worden.
 */
import { useState, useRef, useEffect } from 'react'

export default function PdfImportDebug() {
  const [status, setStatus] = useState(null)   // null | 'bezig' | 'klaar' | 'fout'
  const [progress, setProgress] = useState(null)
  const [result, setResult] = useState(null)
  const [perKlas, setPerKlas] = useState(null)
  ...
  async function handleFile(e) {
    ...
    setResult(null)
    try {
      // Lazy geladen: pdfjs-dist weegt >800kB en mag niet in de hoofdbundel
      // terechtkomen voor leerkrachten die deze (nog experimentele) functie
      // niet gebruiken.
      const { parseKlaslijstPdf, groepeerPerKlas } = await import('../utils/pdfImport')
      const parsed = await parseKlaslijstPdf(file, {
        onProgress: p => setProgress(p),
      })
      setResult(parsed)
      setPerKlas(groepeerPerKlas(parsed.pages))
      setStatus('klaar')
    } catch (err) {
  ...
  const overgeslagen = result ? result.pages.filter(p => !p.klasId) : []

  return (
    <div className="bg-white rounded-2xl shadow p-4 mb-4 border-2 border-dashed border-blue-200">
      <h2 className="font-semibold mb-0.5" style={{ color: '#2C3E50' }}>
        🧪 Importeer PDF — stap 1 (debug: tekst/klas-herkenning)
      </h2>
      <p className="text-xs text-gray-400 mb-3">
        Experimenteel, additioneel aan de CSV-import hierboven. Leest de klaslijst-PDF volledig
        lokaal in en toont enkel wat herkend wordt — schrijft nog niets weg.
      </p>
      ...
      >
        {status === 'bezig' ? '⏳ Bezig met verwerken…' : 'Kies Klaslijst.pdf'}
      </button>
      ...
      {status === 'klaar' && result && (
        <div className="mt-3 space-y-3">
          <p className="text-sm font-semibold text-green-600">
            ✓ {result.numPages} pagina's verwerkt — {Object.keys(perKlas).length} klassen herkend.
          </p>
            ...
            <table className="min-w-full text-xs">
              <thead>
                <tr className="border-b border-gray-200 text-left">
                  <th className="px-2 py-1">Klas</th>
                  <th className="px-2 py-1">Pagina's</th>
                  <th className="px-2 py-1">Naam-items</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(perKlas).sort(([a], [b]) => a.localeCompare(b)).map(([klasId, info]) => (
                  <tr key={klasId} className="border-b border-gray-50">
                    <td className="px-2 py-1 font-semibold" style={{ color: '#2C3E50' }}>{klasId}</td>
                    <td className="px-2 py-1">{info.pageNums.join(', ')}</td>
                    <td className="px-2 py-1">{info.items.length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {overgeslagen.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-3 py-2 text-xs text-yellow-700">
              {overgeslagen.length} pagina('s) overgeslagen (geen cijfer-klascode herkend):{' '}
              {overgeslagen.map(p => `#${p.pageNum} (${p.code ?? 'geen titel'})`).join(', ')}
            </div>
          )}

          <details className="text-xs">
            <summary className="cursor-pointer text-gray-500 font-semibold">Ruwe naam-items per pagina tonen</summary>
            <div className="mt-2 space-y-2 max-h-96 overflow-y-auto">
              {result.pages.map(p => (
                <div key={p.pageNum} className="border border-gray-100 rounded-lg px-2 py-1.5">
                  <p className="font-semibold text-gray-500">
                    Pagina {p.pageNum} — {p.code ?? '(geen titel)'} {p.klasId ? `→ klas ${p.klasId}` : '(overgeslagen)'}
                  </p>
                  {p.items.map((it, idx) => (
                    <p key={idx} className="text-gray-400 font-mono">
                      y={it.y0.toFixed(0)} x={it.x0.toFixed(0)}  "{it.text}"
                    </p>
                  ))}
                </div>
              ))}
...
/**
 * Stap 2 (debug): losstaande knop/upload, toont enkel geëxtraheerde
 * foto-thumbnails per klas — nog GEEN naam-koppeling (stap 3), GEEN
 * Dexie-writes (stap 4). Bewust een aparte knop/state van stap 1, zodat
 * beide stappen onafhankelijk van elkaar getest kunnen worden.
 */
function FotoExtractieDebug() {
  ...
  async function handleFile(e) {
    ...
    setPerKlas(null)
    try {
      const { extractFotosPerPagina, groepeerFotosPerKlas } = await import('../utils/pdfImport')
      const parsed = await extractFotosPerPagina(file, {
        onProgress: p => setProgress(p),
      })
      setResult(parsed)
      setPerKlas(groepeerFotosPerKlas(parsed.pages))
      setStatus('klaar')
    } catch (err) {
      setError(err.message)
      setStatus('fout')
  ...
  return (
    <div>
      <h2 className="font-semibold mb-0.5" style={{ color: '#2C3E50' }}>
        🧪 Importeer PDF — stap 2 (debug: foto-extractie)
      </h2>
      <p className="text-xs text-gray-400 mb-3">
        Nog géén naam-koppeling — enkel om te controleren of de foto's correct uitgesneden worden.
        Traag: elke pagina wordt volledig gerenderd, dit kan een tijdje duren.
      </p>
      ...
      >
        {status === 'bezig' ? '⏳ Bezig met renderen…' : 'Kies Klaslijst.pdf (foto-extractie testen)'}
      </button>
      <input ref={inputRef} type="file" accept=".pdf" className="hidden" onChange={handleFile} />
      ...
      {status === 'klaar' && result && perKlas && (
        <div className="mt-3 space-y-4">
          {(() => {
            const totaalFotos = Object.values(perKlas).reduce((s, k) => s + k.fotos.length, 0)
            return (
              <p className="text-sm font-semibold text-green-600">
                ✓ {totaalFotos} foto's uitgesneden uit {Object.keys(perKlas).length} klassen.
              </p>
            )
          })()}
          ...
          {Object.entries(perKlas).sort(([a], [b]) => a.localeCompare(b)).map(([klasId, info]) => (
            <div key={klasId}>
              <p className="text-xs font-bold text-gray-500 mb-1.5">
                Klas {klasId} ({info.fotos.length} foto's)
              </p>
              <div className="flex flex-wrap gap-2">
                {info.fotos.map((foto, idx) => (
                  <FotoThumbnail key={idx} foto={foto} />
                ))}
              </div>
            </div>
          ))}
...
/**
 * Stap 3 (debug/preview): naam↔foto-matching tegen de bestaande leerlingen
 * in Dexie (ingevoerd via de huidige CSV-import). Read-only — er wordt hier
 * NIETS weggeschreven; dat gebeurt pas in stap 4, achter een bevestigknop
 * die hier nog niet bestaat.
 *
...
function MatchingPreview() {
  ...
  const heeftTwijfelgevallen = preview
    ? Object.values(preview.perKlas).some(info => info.dubbeleNamen.length > 0 || info.mismatch)
    : false

  return (
    <div>
      <h2 className="font-semibold mb-0.5" style={{ color: '#2C3E50' }}>
        🧪 Importeer PDF — stap 3+4 (preview + bevestigde import)
      </h2>
      <p className="text-xs text-gray-400 mb-3">
        Matcht tegen de leerlingen die nu al in Dexie staan. Er wordt pas geschreven na je expliciete
        bevestiging hieronder — tot dan is dit nog altijd enkel een preview.
      </p>
      ...
      >
        {status === 'bezig' ? '⏳ Bezig met matchen…' : 'Kies Klaslijst.pdf (matching-preview)'}
      </button>
      <input ref={inputRef} type="file" accept=".pdf" className="hidden" onChange={handleFile} />
        ...
        <div className="mt-3 space-y-4">
          ...
          {importStatus === 'klaar' && importResultaat ? (
            ...
            <div className="border-t border-dashed border-gray-200 pt-3">
              {heeftTwijfelgevallen && (
                <label className="flex items-start gap-2 text-xs text-red-700 mb-2 cursor-pointer">
                  <input type="checkbox" checked={akkoord} onChange={e => setAkkoord(e.target.checked)} className="mt-0.5" />
                  <span>
                    Ik heb de rode/amber twijfelgevallen hierboven (dubbele namen en/of aantal-mismatch) manueel
                    gecontroleerd en bevestig dat de koppeling correct is.
                  </span>
                </label>
              )}
              <button
                onClick={handleImporteer}
                disabled={importStatus === 'bezig' || (heeftTwijfelgevallen && !akkoord)}
                className="px-4 py-3 rounded-xl font-semibold text-white text-base w-full disabled:opacity-50"
